...
public class PluginSettings implements SettingsHolder {
    ...
    public static final Property<Boolean> DEBUG = newProperty("plugin.debug", true);

    @Comment("Should we use PlaceholderAPI plugin?")
    public static final Property<Boolean> USE_PLACEHOLDER_API = newProperty("plugin.use-placeholder-api", true);

    @Comment("Address where socket listen to messages and send it")
    public static final Property<String> SOCKET_URL  = newProperty("plugin.socket-url", "ws://127.0.0.1:3000");
    ...
    @Comment("Server name. If empty server.properties.name will be used")
